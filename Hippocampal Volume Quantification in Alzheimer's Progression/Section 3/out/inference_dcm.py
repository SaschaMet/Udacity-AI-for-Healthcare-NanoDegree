...
def save_report_as_dcm(header, report, path):
    ...
    # means that bytes encode pixels as R1G1B1R2G2B2... as opposed to R1R2R3...G1G2G3...
...
def get_series_for_inference(path):
    ...
    series_path = [dir for dir, subdirs,
                   files in os.walk(path) if 'HCropVolume' in dir]
    chosen_path = np.random.choice(series_path)
    series_for_inference = [pydicom.dcmread(os.path.join(
        chosen_path, f)) for f in os.listdir(chosen_path)]
